...
The system SHALL allow an authenticated product user to report an artwork or a comment by submitting a structured report reason and optional free-text context against exactly one active moderation target.

#### Scenario: Authenticated user reports artwork
- **WHEN** an authenticated user submits a valid report targeting an existing artwork
- **THEN** the system persists a report linked to that artwork and the reporting user

#### Scenario: Authenticated user reports comment
- **WHEN** an authenticated user submits a valid report targeting an existing comment
- **THEN** the system persists a report linked to that comment and the reporting user
...
The system SHALL automatically hide reported artwork or comment content from public views once the configured active report threshold for that target is reached.
